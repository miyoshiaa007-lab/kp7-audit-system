...
class RecordEntry(BaseModel):
    ...
    position_no: Optional[str] = Field(description="เลขที่ตำแหน่ง เช่น '5693', '3332'")
    academic_standing: Optional[str] = Field(description="วิทยฐานะ เช่น 'ชำนาญการ', 'ชำนาญการพิเศษ'")
    salary: Optional[float] = Field(description="อัตราเงินเดือน (ตัวเลขเท่านั้น ไม่ใส่ลูกน้ำ) เช่น 25190, 33800")
    order_ref: Optional[str] = Field(description="เอกสารอ้างอิง/คำสั่ง เช่น 'คส.สพป.มค.2 ที่ 199/54 ลว. 18 เม.ย. 54'")
...
def normalize_thai_date(date_str: str):
    """
    แปลงวันที่ภาษาไทยทุกรูปแบบ -> YYYY-MM-DD (พ.ศ.) และสร้างคีย์เรียงลำดับ
    ตัวอย่าง: '1 เม.ย. 54' -> ('2554-04-01', 25540401)
    """
...
def extract_data_from_pdf(pdf_bytes: bytes, api_key: str, file_type_hint: str) -> List[dict]:
    ...
    prompt = f"""
    คุณคือผู้เชี่ยวชาญการอ่านและวิเคราะห์ทะเบียนประวัติ ก.พ.7 / ก.ค.ศ.16 ของข้าราชการครูไทย
    ประเภทเอกสาร: {file_type_hint}
    
    คำสั่งการทำงาน:
    1. สกัดข้อมูลประวัติการดำรงตำแหน่งและเงินเดือนทุกแถว (Row) ให้ครบถ้วน 100% ห้ามข้ามแม้แต่แถวเดียว
    2. สำหรับเอกสารเขียนมือ ให้อ่านลายมือภาษาไทย ตัวเลขไทย/อารบิก และสัญลักษณ์ย่ออย่างแม่นยำ
    3. ถอดข้อมูล:
       - วันเดือนปี (date_raw): ถอดตามที่เขียน เช่น '1 เม.ย. 54', '1 ต.ค. 2568'
       - อัตราเงินเดือน (salary): แปลงเป็นตัวเลขเพียวๆ เช่น 25190, 33800
       - เอกสารอ้างอิง (order_ref): เลขที่คำสั่ง วันที่ลงนาม
    4. ส่งออกผลลัพธ์ตาม JSON Schema ที่กำหนดอย่างเคร่งครัด
    """
...
def reconcile_records(records_a: List[dict], records_b: List[dict]):
    """
    เปรียบเทียบข้อมูล 2 ฝั่ง:
    records_a = ไฟล์ 1 (ก.พ.7 อิเล็กทรอนิกส์)
    records_b = ไฟล์ 2 (ก.ค.ศ.16 เขียนมือ)
    """
...
# ==========================================
def export_to_excel(comparison_table, inversions_b):
    wb = openpyxl.Workbook()
    
    # Sheet 1: ตารางเทียบข้อมูล
    ws1 = wb.active
...
if st.button("🚀 เริ่มต้นการสกัดข้อมูลและตรวจสอบความถูกต้อง", type="primary"):
    if not gemini_api_key:
        ...
        with st.spinner("🔍 กำลังประมวลผลด้วย VLM Model และวิเคราะห์ความเที่ยงตรง..."):
            try:
                # 1. สกัดข้อมูลไฟล์ 1
                bytes_e = file_e.read()
                records_e = extract_data_from_pdf(bytes_e, gemini_api_key, "ไฟล์อิเล็กทรอนิกส์ กพ.7")
                
                # 2. สกัดข้อมูลไฟล์ 2
                bytes_m = file_m.read()
                records_m = extract_data_from_pdf(bytes_m, gemini_api_key, "ไฟล์เขียนมือ กคศ.16")
                ...
                # ปุ่มดาวน์โหลด Excel
                excel_data = export_to_excel(comp_table, inv_b)
                st.download_button(
