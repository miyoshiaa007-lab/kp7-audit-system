...
# ==========================================
# 3. VLM Data Extractor Engine (High Precision)
# ==========================================
def extract_pdf_records_precise(pdf_bytes: bytes, api_key: str, model_name: str, hint: str) -> List[Dict[str, Any]]:
    ...
    try:
        model = genai.GenerativeModel(
            ...
            generation_config={
                ...
                "temperature": 0.0  # ปิดความคิดสร้างสรรค์เพื่อให้ตรงกับข้อเท็จจริง 100%
            }
        )
    except Exception:
        model = genai.GenerativeModel(
    ...
    resp = model.generate_content([prompt, pdf_part])
